...
| Confusing taxon `name`, `externalName`, `label`, and `path` when writing SCRIPT steps | Activity-plan SCRIPT helpers (`getOrCreateChild`, `addChild`, `getOrCreate`, etc.) match on the **full `path` field** of the taxon — e.g. `'Invoice/line_items'`, not just `'line_items'`. `name`/`externalName`/`label` are display concerns and aren't used for runtime lookups. See the activity-plan skill's "Document scripting helpers" section. |
