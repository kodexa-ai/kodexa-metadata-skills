...
| Same message for `workspace` against a modern deployment | Workspaces exist only on the legacy backend; the Go API removed the type | Drop `workspace` from the manifest |
